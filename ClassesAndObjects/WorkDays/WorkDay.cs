...
namespace WorkDays
{
    class WorkDay
    {
        ...
        static void Main()
        {
            ...
            Console.WriteLine("This program will check how many workdays there are between {0:dd.MM.yyyy} and {1:dd.MM.yyyy}", currentDate, futureDate);
            Console.WriteLine("\nTotal workdays: {0}", CheckWorkingDays(currentDate, futureDate));
        }

        private static int CheckWorkingDays(DateTime currentDate, DateTime futureDate)
        {
            int count = 0;
            if (currentDate > futureDate)
            {
                DateTime swap = currentDate;
